...
class PairCooldown:
    ...
    Canonical normalization: headshot_knock and normal_knock both map to "knock",
    headshot_elimination and normal_elimination both map to "elimination".  This
    prevents the same physical killfeed from being emitted twice just because the
    icon classifier flipped between frames.  Knock→elimination upgrades still pass
    because their base types differ ("knock" vs "elimination").
    """
    ...
    def _base_event_type(canonical: str) -> str:
        """Collapse subtype variants to a single base key."""
        if "_knock" in canonical:
            return "knock"
        if "_elimination" in canonical:
            return "elimination"
        return canonical  # revive, playzone_elimination, etc. stay distinct
        ...
        if RAPIDFUZZ_AVAILABLE:
            ...
            for existing, ts_e in list(self._entries.items()):
                ...
                if (
                    ...
                    and norm == ec  # exact match on normalised base type
                ):
                    return False
        return True
...
class KillfeedState:
    ...
    def diff_and_commit(self, current: List[str]) -> List[str]:
        ...
        if not self.previous_snapshot:
            # First ever detection: set the baseline silently so we don't
            # re-emit kills that were already on screen before capture began.
            # Bursts after a temporary YOLO gap are handled by find_new_slots'
            # shift alignment + no-overlap fallback (no baseline reset needed),
            # so a single dropped/blank frame can never re-emit old rows or
            # reorder the stack.
