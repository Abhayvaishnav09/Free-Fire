...
from killfeed.roi import StripStabilizer, associate_revive_with_killblocks, detect_strip
...
class KillfeedPipeline:
    ...
    def _capture_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            if roi is None or roi.strip.size == 0:
                ...
                # Do NOT reset the diff baseline on a YOLO miss. A single blank
                # or dropped frame must never cause old rows to be re-emitted or
                # the stack order to change. find_new_slots() handles bursts and
                # post-gap reappearance via shift alignment + no-overlap fallback.
                time.sleep(0.005)
                continue
    ...
    def _process_strip_job(self, job: StripJob) -> None:
        try:
            ...
            # Re-associate YOLO revive icon boxes with full killblock rows so
            # class_name='revive' always uses the complete killfeed crop.
            job.roi.rows = associate_revive_with_killblocks(list(job.roi.rows))
            ...
            # Emit in oldest-first order within the burst so TMS receives the
            # newest kill last.  TMS shows the most recently received event at
            # the top of the killfeed, which is where the newest kill belongs.
            # new_sigs is ordered newest→oldest (position-0 first); reversing
            # it gives oldest→newest, matching the game's visual order on TMS.
