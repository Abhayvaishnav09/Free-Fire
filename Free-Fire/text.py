...
class FreeFireTextDetector:
    ...
    def _best_name_from_half(self, half_image: np.ndarray) -> Tuple[str, float]:
        """Extract the best player-name text from one half of a killfeed row."""
        if half_image is None or half_image.size == 0:
            return "", 0.0
        regions = self.detect_text_regions(half_image)
        if not regions:
            return "", 0.0
        best = self.find_largest_text(regions)
        if best is None:
            best = max(regions, key=lambda r: float(r.get("confidence", 0.0)))
        text = str(best.get("text", "")).strip()
        conf = float(best.get("confidence", 0.0))
        if not text:
            return "", 0.0
        merged_k, merged_v = self._split_merged_player_tags(text)
        if merged_k and merged_v:
            return merged_k, conf
        return text, conf

    def extract_killfeed_sequence_by_halves(self, image: np.ndarray) -> Dict:
        """
        Position-fixed extraction: left zone = killer, right zone = victim.

        Skips the centre weapon-icon band so OCR never reads icon glyphs as
        player names.  Falls back to left-to-right sorting when a half is empty.
        """
        if image is None or image.size == 0:
        ...
        _h, w = image.shape[:2]
        if w < 48:
            return self.extract_killfeed_sequence(image)

        left_end = int(w * 0.42)
        right_start = int(w * 0.58)
        killer, k_conf = self._best_name_from_half(image[:, :left_end])
        victim, v_conf = self._best_name_from_half(image[:, right_start:])
