...
class KillblockDetector:
    ...
    def _detect_killblocks_and_revives_local(self, image):
        ...
        try:
            ...
            from killfeed.roi import consolidate_detection_dicts

            return consolidate_detection_dicts(detections)
        except Exception as e:
            return []
    ...
    def _determine_status_local(self, cropped_image, detected_class=None):
        ...
        """
        # Priority 1: YOLO best.pt revive class — authoritative, no colour checks.
    ...
    def _fuzzy_match_player_name(self, ocr_name: str, min_similarity: float = 0.65) -> tuple:
            ...
            try:
                ...
                if best_match:
                    ...
                    if similarity >= min_similarity:
                        ...
                        # Roster is loaded but OCR result is not close enough to any
                        # registered player.  Reject it — never emit names outside roster.
                        print(f"⚠️  Roster reject: '{ocr_name}' (best '{canonical_name}' {similarity:.2f} < {min_similarity})")
        ...
        if best_match and best_similarity >= min_similarity:
            ...
            if best_match:
                # Roster is loaded but best match is below threshold — reject.
                print(f"⚠️  Roster reject: '{ocr_name}' (best '{best_match}' {best_similarity:.2f} < {min_similarity})")
            return "", best_similarity, False
